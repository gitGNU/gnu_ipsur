...
The estimate \(S\) for \(\sigma\) is called the `Residual standard error` and for the `cars` data is shown a few lines up on the
...
S\approx\) `r tmps`, or we can access it directly from the
`summary` object.
...
[@HH]. The graph is displayed in Figure [[fig-carscipi]].
