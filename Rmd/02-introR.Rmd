...
## Downloading and Installing \(\mathsf{R}\) {#sec-download-install-R}
...
4. [Download this shortcut](http://ipsur.r-forge.r-project.org/book/download/R.exe) and
   ...
   the \(\mathsf{R}\) folder, not inside the folder. Use the
   downloaded shortcut to run \(\mathsf{R}\).
...
\(\mathsf{R}\) and add-on packages, see the [R Installation and Administration manual](http://cran.r-project.org/manuals.html).

## Communicating with \(\mathsf{R}\) {#sec-Communicating-with-R}
...
* \(\mathsf{R}\) Editor (Windows): :: In Microsoft\(\circledR\) Windows,
...
## Basic \(\mathsf{R}\) Operations and Concepts {#sec-Basic-R-Operations}
...
@@latex:\index{wilcox.test@\texttt{wilcox.test}}@@ (see Chapter \@ref(cha-Nonparametric-Statistics)) we get the following:
...
related to
\(\mathsf{R}\). See [here](http://www.r-project.org/mail.html) for an
...
1. Read the [FAQ}(http://cran.r-project.org/faqs.html). Note that
   ...
   learn a lot about the idiosyncrasies of \(\mathsf{R}\).
