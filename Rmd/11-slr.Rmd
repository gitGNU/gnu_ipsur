...
of \(x\). This turns out to be a difficult problem[^1], so
...
[^1]: Yogi Berra once said, "It is always difficult to make
...
\@ref(cha-multivariable-distributions) that if \( (X,Y) \sim
...
[^2]: In practical terms, this constant variance assumption is
...
```{r, echo=FALSE,label="slr-philosophy",fig.cap='(ref:cap-philosophy)'}
...
```r{example, name="Speed and stopping distance of cars"}
...
command in the `ggplot2` package [@ggplot2].
...
\beta_{1} \). See Appendix \@ref(sec-multivariable-calculus) which tells us that
...
**Remark.** The formula for \(b_{1}\) in Equation
...
```{r,label="carline",fig.cap='(ref:carline)'}
...
[^3]: Be careful not to confuse the mean square error \(S^{2}\)
...
`head(cars)` in Example \@ref(ex:speed-and-stopping); it was \(\mathtt{dist}
...
\@ref(fig:normal-q-q-plot-cars). There we see that the distribution of
the residuals appears to be mound shaped, for the most part. We can
plot the order statistics of the sample versus quantiles from a
\(\mathsf{norm}(\mathtt{mean}=0,\,\mathtt{sd}=1)\) distribution with
the command `plot(cars.lm, which = 2)`, and the results are in Figure
...
departure from normality in that the dots show systematic clustering
on one side or the other of the line. The points on the upper end of
the plot also appear begin to stray from the line. We would say there
is some evidence that the residuals are not perfectly normal.

```{r,label="normal-q-q-plot-cars",fig.cap='(ref:cap-normal-q-q-plot-cars)'}
plot(cars.lm, which = 2)
```

(ref:cap-normal-q-q-plot-cars) \small Used for checking the normality assumption. Look out for any curvature or substantial departures from the straight line; hopefully the dots hug the line closely.
...
[^4]: Cook's distances are actually defined by a different
