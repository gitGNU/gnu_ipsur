...
bayesian material in later editions of this book. \@ref(fig:stripcharts)
...
```{r, label="3D-scatterplot-trees",fig.cap='(ref:3D-sctterplot-trees)'}
...
In Equation \eqref{eq-mlr-single-yhat-matrix} we wrote $Y(\mathbf{x}_0)=\mathbf{x}_0^{\mathrm{T}}\mathbf{b}$, and
...
\), when the null hypothesis is true. We saw in Section \@ref(sub-mlr-interval-est-params) that
\begin{equation}
...
[fn:fn-contrast] That is, R by default handles contrasts
...
```{example, name="mlr-trees-poly-no-rescale"}
