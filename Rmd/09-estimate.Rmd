...
```{example, label="how-many-fish"}
Suppose we have a small pond in our backyard,
...
(ref:cap-capture-recapture) \small A plot of maximum likelihood for the capture-recapture experiment.

\bigskip

```{example, label="bass-bluegill"}
...
value \(\hat{p}=\overline{x}\) is located at a minimum[^estimate-02]
...
[^estimate-02]: We can tell from the graph that our value of \(\hat{p}\)
...
(ref:cap-species-mle) \small Species maximum likelihood.
...
* MLEs are just one of _many_ possible estimators. One of the more
  ...
  Casella and Berger [@Casella2002] for more.
...
There is functionality in the `distrTest` package [@distrTest] to
