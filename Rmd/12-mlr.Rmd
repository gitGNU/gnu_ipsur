...
```{r, label="3D-scatterplot-trees",fig.cap='(ref:3D-scatterplot-trees)'}
...
invertible[^1], we may solve the equation for \(\upbeta\) to
...
[^1]: We can find solutions of the normal equations even when
...
\end{equation}
The degrees of freedom for the Student's \(t\) distribution[^2]
are the same as the denominator of \(S^{2}\).

[^2]: We are taking great leaps over the mathematical
...
independent variables in the model[^3].

[^3]: In other words, a variable might be highly significant
...
[^4]: Rescaling the data gets the job done but a better way to
...
**Remark.** When a model includes a quadratic term for an independent
...
purposes[^5].  We will leave the `Girth` variable alone,
...
[^5]: This procedure of replacing a continuous variable by a
...
automatically without input from the user[^6]. Indeed, the
...
[^6]: That is, R by default handles contrasts
