...
```{r, echo=FALSE, label="diagram", fig.cap='(ref:cap-dagram)'}
require(diagram)
par(mex = 0.2, cex = 0.5)
...
[^1]: The seasoned R user can get the job done
...
in other words, a subset of the sample space[^2]. After the
...
[^2]: This naive definition works for finite or countably
...
**Note.** When the `prob` package [@prob] loads you will notice a message:
...
[^3]: Conditional probability in this case is defined by
...
```{r, label="twodiceAB",fig.cap='(ref:cap-twodiceAB)'}
A <- rolldie(2)
B <- subset(A, X1==X2)
C <- subset(A, X1+X2 > 7)
B$lab <- rep("X", dim(B)[1])
...
(Continued, see Example [[exa-unbalanced-coin]]). It was
...
(Continued from Example [[exa-misfiling-assistants]]). We store the
...
We study these in Chapter [[#cha-Discrete-Distributions]].
...
reason[^4], random variables with supports like \(Z\) are
called *continuous random variables*, to be studied in Chapter
[[#cha-Continuous-Distributions]].

[^4]: This isn't really the reason, but it serves as an
